...
function handleEditClick(){ //allows user to edit
    makeEditable();
    hideButtons();
...
function handleNewClick(){ //allows user to create new entry
    makeEditable();
    hideButtons();
    blankFields();
...
function handleRentClick(){ //decreases rental inventory
    myBook.numAvlb--;
    document.getElementById("boolAvlb").value=myBook.numAvlb;
    putBook(myBook.id);
}

function handleReturnClick(){ //increases rental inventory
    myBook.numAvlb++;
...
function handleEditSave(id){ //calls book id that user wants to edit
    putBook(id);
    makeReadOnly();
    showButtons();
}
...
function populateForm(){ //displays item information to use
